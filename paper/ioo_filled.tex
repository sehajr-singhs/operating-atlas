...
\centering
\small
\caption{In-distribution test error by router input. One expert bank, one budget, identical expert inputs; only the router input differs. Percentages and paired $p$-values are against the raw-state router over common seeds.}
\label{tab:results}
\begin{tabular}{lrr}
\toprule
router input & \multicolumn{2}{c}{C-MAPSS (cycles$^2$)} \\
 & error & vs raw \\
\midrule
monolith & 299.3\,$\pm$\,12.7 & +0.5\% \\
raw state & 297.9\,$\pm$\,3.7 & --- \\
invariants $(R,\Pen)$ & 296.0\,$\pm$\,2.8 & -0.6\% \\
raw $+$ invariants & 295.2\,$\pm$\,5.3 & -0.9\% \\
$\Trr V,\ \log\det V$ & 290.5\,$\pm$\,1.3 & \textbf{-2.5\%} \\
local activity & 301.7\,$\pm$\,5.7 & +1.3\% \\
random projection & 299.2\,$\pm$\,2.9 & +0.4\% \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering\small
\caption{Whitened-increment diagnostics. A correctly specified diffusion gives excess kurtosis $0$, a $5\sigma$ ratio of $1\times$, small KS distance and whitened s.d.\ $1$.}
\label{tab:diffusion}
\begin{tabular}{lrrrr}
\toprule
system & excess kurtosis & $>5\sigma$ vs Gaussian & KS (radial) & whitened s.d. \\
\midrule
pmsm & 215.5 & 8,641$\times$ & 0.740 & 0.871 \\
cmapss & -1.1 & 0$\times$ & 0.170 & 0.997 \\
\bottomrule
\end{tabular}
\end{table}
