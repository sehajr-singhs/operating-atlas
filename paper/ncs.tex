...
\begin{abstract}
Industrial telemetry lives on a coupled trajectory, and the relations between channels carry information that no single channel contains. We construct a nine-descriptor atlas of those relations for any record of multi-channel telemetry. Eight descriptors are computed on within-record ranks and are therefore exactly invariant to any independent monotone recalibration of each channel, verified to machine precision on real data. The ninth, the signed L\'evy area, is the unique descriptor odd under time reversal and captures causal lag that no symmetric statistic can reach. On seven real industrial systems (309 records), the atlas identifies the source system at $99.4\%$ accuracy versus $96.1\%$ for per-channel marginals; under simulated re-instrumentation, the atlas holds at $99.0\%$ while marginals collapse to $80.3\%$. In controlled simulated fleets of 48 machines replicated across three robotic platforms, an autoencoder trained on raw telemetry outperforms the atlas on clean data but collapses to chance under re-instrumentation, while the atlas is numerically unchanged. The method requires a minimum of $\sim10^3$ samples per window; its value is in identification, auditing and transfer under changing instrumentation, not in prediction.
\end{abstract}
...
identification, but the prediction we state is that it would collapse
