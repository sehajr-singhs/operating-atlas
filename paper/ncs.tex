...
% Note: figure paths relative to paper/; copy fig*.pdf here for journal submission
...
deployment rather than about the method. The atlas and a
...
\end{minipage}
\caption{Atlas construction. Steps 1--2 are $O(d^2 n)$ with no pairwise distance statistic.
Step 3 produces a fixed-length signature regardless of $d$. Step 4 is the identification protocol.}
\label{tab:algorithm}
\end{figure}
...
(Tables~\ref{tab:fleetctl} and~\ref{tab:fleetwk}) are omitted because those
...
\caption{Extended Data Figure 1. The even/odd decomposition under time
